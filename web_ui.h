...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Label du groupe sur la première cellule */
  .grp-label {
    position: absolute;
...
<script>
  ...
  function showTab(name, btn) {
    ...
    if (name === 'monitor') { if (!monitorInterval) startMonitor(); }
    else stopMonitor();
  ...
  function startMonitor() {
    buildGrids();
    updateDmx();
    monitorInterval = setInterval(updateDmx, 200);
    // Pause polling when browser tab goes to background
  ...
  let dmxFetching = false;
  async function updateDmx() {
    if (dmxFetching) return;
    dmxFetching = true;
    try {
      const r = await fetch('/api/dmx');
      const { in: vi, out: vo } = await r.json();
      for (let i = 0; i < 512; i++) {
        setCell('in',  i+1, vi[i]||0);
        setCell('out', i+1, vo[i]||0);
      }
    } catch(e) {}
    finally { dmxFetching = false; }
  }
  ...
  function buildTestGroups() {
    ...
    config.mappings.forEach((m, ri) => {
      ...
      let html =
        ...
        + '<button class="test-btn zero" onclick="setGroup(' + ri + ',0)">\u25bc Zéro</button>'
